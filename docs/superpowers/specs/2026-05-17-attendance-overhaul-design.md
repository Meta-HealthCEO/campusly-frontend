...
A grid view scoped to the selected class:
...
- **Week view:** Mon–Fri columns by default (configurable as a future option; not in scope). Cell content: single status symbol (`✓ P` / `A` / `L` / `E`) + colour. Tall enough to show a small note indicator if a note exists on that record.
- **Month view:** more compressed — single coloured dot per cell, hover/tap reveals the status.

**Navigation:** Prev/Next period arrows in the tab header (steps by 1 week in week view, 1 month in month view). "Today" button snaps the visible period to include today.
...
**Cells are clickable** → opens a base-ui `<Dialog>` scoped to that day, allowing inline edit of just that day's register (subset of the Today tab's controls — per-student status buttons + notes, save button). Closes back to the grid; the grid refreshes the affected cells. Avoids forcing the user to switch tabs to fix one wrong mark.

**Empty cells** (no register taken that day) render as a faded `–` and are also clickable to take the register for that day. Pre-fills all students as present.

### 4. PDF export

**Export PDF button** in the page header, always visible:

- **In Today tab:** exports the selected day's register for the selected class. PDF layout: school name + class label + date + period header; student list table (admission # | name | status | note); summary totals; signature lines at the bottom (teacher signature + date) — the bit paper-based schools need for staff-room sign-off.
- **In History tab:** exports the visible grid as a PDF table (students × dates). Includes per-student % and per-day % rows; class label and date range in the header.
...
- `src/components/attendance/AttendanceDayEditDialog.tsx` — popover/dialog used by History-tab cell clicks.
...
User switches to History tab
  ...
  → useAttendanceHistory loads /attendance/class/:classId?dateFrom=...&dateTo=...
    (or /attendance/report if that endpoint serves this better — confirm during plan)
  → renders grid

User clicks a cell in History grid
  → opens AttendanceDayEditDialog with that classId+date
  → dialog uses same /attendance/class/:classId?date=... + /attendance/bulk
  → on save, refreshes the affected cell(s)

User clicks Export PDF
  → useAttendanceExport posts to /api/attendance/export with classId, date or dateFrom/dateTo, format=pdf
...
- Clicking a history cell opens a small edit dialog scoped to that day; saving updates the cell without reload.
- Empty (un-taken) history cells render as `–` and are clickable to take that day's register pre-filled as all present.
- The Export PDF button works from both tabs — single-day register from Today, grid PDF from History — and the resulting file opens cleanly in a browser/desktop PDF viewer with the school name, class label, date(s), and signature line(s).
...
- Date-range listing for the History tab: does `GET /attendance/class/:classId` already accept `dateFrom`/`dateTo`? If yes, use it. If no, extend it (small) — `/attendance/report` may also fit but reads as a different shape.
