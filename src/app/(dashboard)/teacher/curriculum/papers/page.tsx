...
import { FileText, Trash2, Eye, Calendar, Clock, Loader2 } from 'lucide-react';
...
export default function GeneratedPapersPage() {
  ...
  const handleViewPaper = (id: string) => {
    router.push(`${ROUTES.TEACHER_CURRICULUM_PAPERS}/${id}`);
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="View and manage AI-generated exam papers with diagrams"
      />
      ...
      {loading ? (
        ...
        <EmptyState
          icon={FileText}
          title="No Papers Yet"
          description="Generate your first paper with AI from the AI Studio to get started."
        />
      ) : (
        <div className="space-y-3">
          {papers.map((paper) => (
            <Card key={paper.id} className="cursor-pointer transition-colors hover:bg-muted/30" onClick={() => handleViewPaper(paper.id)}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold truncate">{paper.subject}</h3>
