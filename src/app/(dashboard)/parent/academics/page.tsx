...
import { mockStudents, mockStudentGrades, mockHomework } from '@/lib/mock-data';
...
const assessmentColumns: ColumnDef<StudentGrade, unknown>[] = [
  ...
  {
    accessorKey: 'assessment.subject.name',
    header: 'Subject',
  },
  {
    accessorKey: 'assessment.type',
    header: 'Type',
    cell: ({ row }) => {
      const type = row.original.assessment.type;
      const styles: Record<string, string> = {
        test: 'bg-blue-100 text-blue-800',
        exam: 'bg-purple-100 text-purple-800',
        assignment: 'bg-amber-100 text-amber-800',
        project: 'bg-emerald-100 text-emerald-800',
        quiz: 'bg-gray-100 text-gray-800',
      };
      return (
  ...
  },
  {
    ...
    header: 'Marks',
    cell: ({ row }) =>
      `${row.original.marks} / ${row.original.assessment.totalMarks}`,
  },
  {
  ...
  },
  {
    ...
    header: 'Date',
    cell: ({ row }) => formatDate(row.original.assessment.date),
  },
];
...
function getSubjectSummary(studentId: string) {
  const grades = mockStudentGrades.filter((g) => g.studentId === studentId);
  const subjectMap = new Map<
    string,
    { name: string; grades: number[]; latestGrade: number }
  >();

  grades.forEach((g) => {
    const subjectName = g.assessment.subject.name;
  ...
  return Array.from(subjectMap.values()).map((subject) => {
    const avg = Math.round(
      subject.grades.reduce((s, g) => s + g, 0) / subject.grades.length
    );
    const trend =
      subject.grades.length >= 2
        ? subject.grades[subject.grades.length - 1] -
          subject.grades[subject.grades.length - 2]
        : 0;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Track your children's academic performance, grades, and homework."
      />
      ...
      <Tabs defaultValue={parentChildren[0]?.id ?? ''}>
        ...
        {parentChildren.map((child) => {
          const studentGrades = mockStudentGrades.filter(
            (g) => g.studentId === child.id
          );
          const subjectSummary = getSubjectSummary(child.id);
          const overallAvg =
            studentGrades.length > 0
              ? Math.round(
                  studentGrades.reduce((s, g) => s + g.percentage, 0) /
            ...
            <TabsContent key={child.id} value={child.id} className="space-y-6">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm text-muted-foreground">
                        Overall Average - {child.user.firstName}
                      </p>
              ...
              <div>
                ...
                {subjectSummary.length > 0 ? (
                  ...
                  <EmptyState
                    icon={GraduationCap}
                    title="No grades yet"
                    description="Grades will appear here once assessments are completed."
                  />
                )}
              </div>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Detailed results for all assessments
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {studentGrades.length > 0 ? (
                    ...
                    <EmptyState
                      ...
                      description="No assessment results available yet."
                    />
                  )}
                </CardContent>
              </Card>
