...
import { classSubjects, weightingLines, type WeightingLine } from '@/lib/weighting-summary';
...
export function GradebookWeightingsTab({ gradeId, subjects, term, canEdit, onSaved }: Props) {
  ...
  const { matrices, loading, refetch } = useGradeWeightings(gradeId, ids);
              ...
              <div key={subject.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0 space-y-1.5">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {current && !current.set ? <StatusChip status="due" label="Not set" /> : null}
                  </div>
