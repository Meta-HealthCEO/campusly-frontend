...
export function useClassroomSessions(initialFilters?: ClassroomSessionFilters) {
  ...
  const fetchSessions = async (filters?: ClassroomSessionFilters) => {
    ...
    try {
      ...
      setSessions(Array.isArray(raw) ? raw : raw.sessions ?? raw.data ?? []);
    } catch (err: unknown) {
      console.warn('Failed to fetch classroom sessions', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const cancelSession = async (sessionId: string): Promise<void> => {
    await apiClient.delete(`/classroom/sessions/${sessionId}`);
  };

  const startSession = async (sessionId: string): Promise<VirtualSession> => {
    const response = await apiClient.post(`/classroom/sessions/${sessionId}/start`);
    return response.data.data ?? response.data;
  };

  const endSession = async (sessionId: string): Promise<VirtualSession> => {
    const response = await apiClient.post(`/classroom/sessions/${sessionId}/end`);
    return response.data.data ?? response.data;
  };
