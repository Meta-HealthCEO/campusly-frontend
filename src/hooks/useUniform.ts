...
export function useUniformItems() {
  ...
  const fetchItems = useCallback(async (category?: string) => {
    try {
      ...
      const response = await apiClient.get('/uniform/items', { params });
...
export function useUniformOrders() {
  ...
  const fetchOrders = useCallback(async (status?: string) => {
    try {
      ...
      const response = await apiClient.get('/uniform/orders', { params });
      const raw = response.data.data ?? response.data;
...
export function useSecondHand() {
  ...
  const fetchListings = useCallback(async (status?: string) => {
    try {
      const params: Record<string, string> = { schoolId };
      if (status && status !== 'all') params.status = status;
      const response = await apiClient.get('/uniform/second-hand', { params });
      const raw = response.data.data ?? response.data;
...
export function usePreOrders() {
  ...
  const fetchPreOrders = useCallback(async (status?: string) => {
    try {
      const params: Record<string, string> = { schoolId };
      if (status && status !== 'all') params.status = status;
      const response = await apiClient.get('/uniform/pre-orders', { params });
      const raw = response.data.data ?? response.data;
...
export function useLowStock() {
  ...
  const fetchLowStock = useCallback(async () => {
    try {
      const response = await apiClient.get('/uniform/low-stock', {
        params: { schoolId },
      });
      const raw = response.data.data ?? response.data;
...
export function useSizeGuide(itemId: string) {
  ...
  const fetchSizeGuide = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await apiClient.get(`/uniform/items/${itemId}/size-guide`);
      const raw = response.data.data ?? response.data;
